# ReRoute — Refined Feature Requirements Document (v2.1)
...
Legend: **Real** = build fully working for demo. **Mocked** = simulated for demo. **TBD** = team must confirm before building. **[NEW]** = introduced by the update doc. **[CHG]** = existing requirement changed by the update doc. **[CUT]** = considered but placed out of scope. **[OUT OF SCOPE]** = explicitly not built for this demo.
...
- FR-8-2: **[CHG]** Users can send/accept friend requests (unchanged mechanism, relocated UI). **[CUT]** A separate asymmetric "follow" mechanic was considered but is **out of scope** for this build — the `friendships` table only models a symmetric, request-based relationship (pending/accepted/blocked), and no `follows` entity exists in the data model. Friend request/accept is the only relationship mechanism to build against.
...
**Build scope:** Friend requests (send/accept) — Real; Follow — Out of scope (no data model support); Discover feed + filters — Real; Starring — Real; Clone itinerary — Real.
